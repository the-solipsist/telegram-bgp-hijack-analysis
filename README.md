...
Under Section 69A of the IT Act, MEIT directs domestic internet service providers (ISPs) to block access to specific online resources. Standard blocking procedures involve DNS sinkholing, SNI (Server Name Indication) filtering, or IP blocking to prevent domestic subscribers from accessing the platform.
...
2. A total of **33 unique prefixes and sub-prefixes** (covering 16 Telegram parent ranges) were hijacked. *(The `bgp_updates_18101.json` covers 07:00-22:00 UTC with 81,925 total updates, of which 733 (0.9%) are for hijacked prefixes — the rest cover RCom's legitimate prefixes. The 33 prefix-specific files add 17,987 additional updates for deeper timeline analysis.)*
